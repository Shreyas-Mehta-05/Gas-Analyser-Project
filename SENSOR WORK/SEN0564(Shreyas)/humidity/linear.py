...
# Create separate dataframes for 30% and 60% humidity
df_30 = pd.DataFrame({
    'Temperature': df_combined['Temperature'],
    'Humidity': 30,  # Assign 30% humidity
    'Rs_Ro': df_combined['Ratio_30%']
})

df_60 = pd.DataFrame({
    'Temperature': df_combined['Temperature'],
    'Humidity': 60,  # Assign 60% humidity
    'Rs_Ro': df_combined['Ratio_60%']
})
df_85 = pd.DataFrame({
    'Temperature': df_combined['Temperature'],
...
df_final = pd.concat([df_30, df_60,df_85], ignore_index=True)
...
ax.plot_surface(T_grid, H_grid, Z, alpha=0.5, color='blue', label='Surface Fit')
